...
export default function FormCreatePost() {
    ...
    const { getValues, resetField } = useFormContext<HookFormCreatePost>();

    function createCallbackOnEnter(name: FieldContentOfPostIsArray) {
        dispatch(
            setFieldContentOfPostIsArray({
                name,
                value: (getValues(name) as string).trim()
            })
        );
        resetField(name);
    }
