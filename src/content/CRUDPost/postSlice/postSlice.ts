...
import { ListFieldsName } from '../components/ModalCreatePost';
...
export type FieldContentOfPostIsArray =
    | 'timeWorking'
    | 'salaryInformation'
    | 'workLocations';
...
export const postSlice = createSlice({
    ...
    reducers: {
        setFieldContentOfPostIsArray: (
            state,
            {
                payload
            }: PayloadAction<{ name: FieldContentOfPostIsArray; value: string }>
        ) => {
            if (payload) {
                state.formCreate[payload.name].push(payload.value);
            }
        }
    }
});

export const { setFieldContentOfPostIsArray } = postSlice.actions;
